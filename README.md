...
| 送達結果不明（逾時／連線重置） | 200，**不自動重送** |
| 結構不是 Update | 200（重送也不會變好） |
| secret 不符 | 401 |
| 內容過大 / 壞掉的 JSON | 413 / 400 |
| 拿不到執行權、要讓路、明確的送出失敗、處理中途失敗 | 503（Telegram 稍後重送） |

#### 送達保證

提供的是**耐久去重 + 抗重複送達**，不是 exactly-once —— Telegram 的
`sendMessage` 沒有呼叫端冪等鍵，所以「請求送出去了但拿不到回應」時無法證明
對方收到沒有。明確失敗會重試；**只有真正模糊的網路結果才 fail closed**
（寧可極少數情況下漏一則，也不要讓使用者收到兩則一樣的健康建議）。
細節見 [docs/telegram-webhook.md](docs/telegram-webhook.md)。

#### 同一個人的訊息照順序處理

每個使用者有自己的對話通道（`resource_locks` 租約）＋「不可以超車比自己早、
還沒結案的訊息」的檢查，所以澄清回覆不會在問題本身落地之前被處理。
不同使用者完全併行。
