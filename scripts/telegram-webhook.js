...
try {
  ...
  if (action === 'set') {
    ...
    await call('setWebhook', {
      ...
      // 生效的。在那之前還有一段極短的窗口：Telegram 若用多條 HTTPS 連線
      // 同時投遞，N+1 有機會比 N 更早抵達 claim，而那時候資料庫裡還沒有
      // 任何 N 的紀錄可以拿來擋。
      //
      // max_connections=1 讓 Telegram 一次只送一則，把那個 pre-claim 窗口
      // 關掉。對這個低流量的私人健康 bot 來說，對話順序的正確性比傳輸層的
      // 平行度重要得多。
      //
      // ⚠️ 這**不是**唯一的正確性機制，也不可以被當成唯一的：
      // 每次執行各自的 attemptId、對話鍵與對話通道、租約與圍欄、
      // 過期工作的原子終結、送達狀態機（含模糊送達不重送）——
      // 全部仍然必要、全部維持不變。這一行只是把它們涵蓋不到的那一小段
      // 窗口也補起來。
      max_connections: 1,
    });
    console.log(`✅ 已註冊：${url}`);
