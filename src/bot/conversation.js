...
export function looksLikeQuestion(text) {
  ...
  // 「…嗎」是很強的疑問標記，單獨成立。
  if (/嗎\s*[。.!！]?\s*$/.test(t)) return true;
  // 「吧／呢／嘛」不是 —— 它們同樣常出現在**陳述**與**祈使**裡：
  //「好吧」「記錄一下吧」「今天就這樣吧」「我很累呢」「今天真的很累嘛」。
  // 所以這三個字要再看到別的疑問證據才算問句。
  if (/(吧|呢|嘛)\s*[。.!？!]?\s*$/.test(t)
      && /(為什麼|為何|怎麼|如何|多少|哪|幾|是不是|會不會|有沒有|能不能|可不可以|難道|還要|還能|還是)/.test(t)) {
    return true;
  }
...
export function mentionsLoggableEvent(text) {
  const t = String(text ?? '');
  // 「酒吧」「這杯是酒吧的調酒」裡的「酒」不是在記錄飲酒事件。
  // 先把會誤觸的詞遮掉，再比對。
  const masked = t.replace(/酒吧|酒店|酒精濃度|酒駕/g, '');
  return /(喝酒|喝了|喝完|飲酒|吃了|宵夜|熬夜|睡得?晚|沒睡|失眠|生病|不舒服|感冒|發燒|壓力|加班|出差|旅行|時差|按摩|三溫暖|運動|重訓|跑步|練)/
    .test(masked);
}

/**
 * 明確的記錄指令（「幫我記錄喝酒吧」「記一下我昨晚熬夜」）。
 *
