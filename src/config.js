...
export const TELEGRAM_BOT = {
   ...
   * 同一個使用者的「對話通道」租約長度。
   *
   * 同一個人的訊息必須照順序處理（澄清回覆不可以在問題本身落地之前就被處理），
   * 所以一次只能有一則訊息握著這個人的通道。用既有的 resource_locks，
   * 所以它自帶 owner、到期、圍欄與過期回收 —— 崩潰不會永久卡住這個人。
   *
   * 要比最壞情況的處理時間長（一則訊息最多兩次 LLM 呼叫），但不能太長，
   * 否則一個死掉的執行會讓使用者等到租約到期。
   */
  USER_LANE_TTL_MS: 3 * 60_000,

  /**
   * 「更早的訊息」還能擋路多久（TG-R04）。
   *
   * 只看「還沒到終局」會有一個尾巴：某一則如果永遠沒被重送（Telegram 放棄
   * 重試、或崩潰後沒人再碰），它會**永遠**擋住這個對話後面的每一則訊息。
   *
   * 所以只有租約還活著、或剛過期不久的更早訊息才有資格擋路。這是刻意的
   * 取捨：極端情況下容許一次順序顛倒，換取「使用者的 bot 不會永久啞掉」。
   * 設得比認領租約長很多，正常的重送與接手完全不受影響。
   */
  ORDER_BLOCK_GRACE_MS: 30 * 60_000,
  // LLM 產生回答的 token 上限
 ...
 * ## 重疊天數為什麼是 5
 *
 * WHOOP 集合端點只能用資源的 start 時間過濾，沒有 updated_at 過濾。
 * 一筆資料事後被重新評分時 start 不會變，所以唯一能抓到「舊資料的新版本」
 * 的方法就是重讀最近一段。實務上：
 *   · 睡眠 / 恢復的評分通常在結束後數小時內完成，偶爾隔天
...
export const WHOOP_RECONCILE = {
  RESOURCES: ['sleep', 'recovery', 'cycle', 'workout', 'body_measurement'],
  /** 第一次對帳（沒有水位）往回抓多少天。更早的歷史交給明確的 backfill。 */
  INITIAL_WINDOW_DAYS: 45,
  /** 每一種資源的重疊天數。 */
  OVERLAP_DAYS: { sleep: 5, recovery: 5, cycle: 5, workout: 5 },
  /** 一輪、一種資源最多抓幾頁（25 筆／頁）。用完就存續傳 token，水位不前進。 */
  MAX_PAGES_PER_RUN: 8,
  ...
  /** 一輪最多對幾個 ACTIVE 墓碑做單筆 GET 檢查（純診斷）。 */
  MAX_TOMBSTONE_CHECKS_PER_RUN: 5,
  /** 同一個墓碑多久才重新檢查一次。 */
  TOMBSTONE_RECHECK_MS: 24 * 60 * 60_000,
  /**
   * 判定「本地有、遠端沒有」時，窗的兩端各縮多少：邊界上的資源可能因為
