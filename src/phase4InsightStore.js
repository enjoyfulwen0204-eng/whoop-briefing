...
export function createPhase4InsightStore(core,entities) {
  ...
  function promotion(context,items,status) {
    const qualified=items.filter(({item:{row:i},run:{row:r}})=>{
      ...
      return r.promotion_confound_version==='unknown-promotion-confound-v1'
        ...
        && Date.parse(r.window_end_utc)<=Date.parse(timestamp())
        && Date.parse(r.window_end_utc)>=Date.parse(timestamp())-90*86400000;
    });
    ...
    if(status==='SUPPORTED') {
      const supported=qualified.filter(({item:{row:i},run:{row:r}})=>r.multiple_testing_family && i.adjusted_significance!=null
        && i.adjusted_significance<=.10 && r.window_end_utc>=new Date(Date.parse(timestamp())-30*86400000).toISOString());
  ...
  async function read(context,insightId,{history=false}={}) {
    return core.run(context,async()=>{
      const artifact=await core.artifact(context,'health_insights',{id:insightId});
      const r=artifact.row;
      if(!history&&(r.status==='RETIRED'||r.lifecycle_disposition||r.expires_at<=timestamp()))fail('PHASE4_INSIGHT_NOT_CURRENT');
  ...
  async function appendRevision(context,row,{status,disposition=null,claim,supportingEvidenceIds=[],contradictingEvidenceIds=[],reason}) {
    ...
    if(['EMERGING','SUPPORTED'].includes(status))promotion(context,supporting,status);
  ...
  async function create(context,{identity,claim,evidenceContractVersion,supportingEvidenceIds,expiresAt,creationKey,supersedesId=null}) {
    return core.run(context,async()=>{
      if(!creationKey||!evidenceContractVersion||!Number.isFinite(Date.parse(expiresAt))||expiresAt<=timestamp()
        || Date.parse(expiresAt)>Date.parse(timestamp())+90*86400000)fail('PHASE4_INSIGHT_CREATE_INVALID');
      ...
      const r={...core.envelope(context,'health_insights',[creationKey]),privacy_artifact_id:artifactId,
        ...
        first_detected_at:timestamp(),insight_key:key,evidence_contract_version:evidenceContractVersion,expires_at:expiresAt,
      ...
      await appendRevision(context,r,{status:'HYPOTHESIS',claim,supportingEvidenceIds,reason:'CANDIDATE_EVIDENCE'});
      ...
      return {...await read(context,r.id),created:true};
    });
  }
  async function transition(context,{insightId,expectedRevision,status,disposition=null,claim,supportingEvidenceIds,contradictingEvidenceIds=[],reason,refresh=false}) {
    ...
    return core.run(context,async()=>{
      ...
      const revision=await appendRevision(context,row,{status,disposition,claim,supportingEvidenceIds,contradictingEvidenceIds,reason});
      const changed=await client.execute({sql:`UPDATE health_insights SET status=?,lifecycle_disposition=?,statement=?,current_revision=?,
        ...
        args:[status,disposition,claim,revision.row.revision,timestamp(),status==='RETIRED'?timestamp():null,
