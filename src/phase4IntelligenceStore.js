...
import { assessDataQuality, benjaminiHochberg, buildPersonalBaseline, evaluateJournalAssociation,
  evaluateMeaningfulChange, recencyWeight } from './phase4Intelligence.js';
...
export function createPhase4IntelligenceStore(core, entities, episodes, insights) {
  ...
  async function activeEpisode(context,identity) {
  ...
  async function recentResolvedEpisode(context,identity,direction) {
    ...
    if(!row||core.now().getTime()-Date.parse(row.resolved_at)>7*86400000)return null;
    if(!readableRow(row))fail('CONTENT_REDACTED');
    await core.validateStoredGraph(context,'observation_episodes',row.privacy_artifact_id);
    return row;
  }
  ...
  async function durableEvidence(context,{metricKey,sourceSet,baseline,quality,calculation,asOfUtc}) {
    ...
    const item=await entities.append(context,'evidence_items',{run_id:run.row.run_id,item_key:itemKey,
      ...
      recency_weight:recencyWeight((Date.parse(asOfUtc)-Date.parse(sourceSet.current.observedAt))/86400000),causal_status:'ASSOCIATION_ONLY',
      provenance_json:{method:'CURRENT_VS_ROBUST_BASELINE',baseline_version:baseline.version,registry_version:INTELLIGENCE_VERSIONS.registry,
        ...
        severe_single:calculation.severeSingle,semantic_hash:calculation.semanticHash},
  ...
  async function linkMembership(context,episode,item,sourceSet,baseline,calculation) {
    ...
    const observation=prior?await core.artifact(context,'episode_observations',{episode_id:episode.row.episode_id,observation_key:observationKey})
      :await entities.append(context,'episode_observations',{episode_id:episode.row.episode_id,observation_key:observationKey,
        ...
        quality:calculation.confidence>0?'QUALIFIED':'INSUFFICIENT',added_at:sourceSet.current.observedAt},[item.ref,...sourceSet.refs]);
    const evidence=await entities.append(context,'episode_evidence',{episode_id:episode.row.episode_id,evidence_item_id:item.row.evidence_item_id,
      episode_revision:episode.row.revision,relationship_type:'SUPPORTING',linked_at:sourceSet.current.observedAt},[item.ref]);
  ...
  function episodeData(context,sourceSet,calculation,asOfUtc) {
    const contract=phase4Metric(calculation.metricKey);
    return {episode_type:'METRIC_DEVIATION',severity:calculation.severity,current_confidence:calculation.confidence,
  ...
  async function episodeForEvidence(context,{identity,active,sourceSet,calculation,item,asOfUtc}) {
    ...
    if(!active) {
      const reopened=await recentResolvedEpisode(context,identity,calculation.direction);
      const opened=await episodes.open(context,{identity:{...identity,direction:calculation.direction},data:episodeData(context,sourceSet,calculation,asOfUtc),
        evidenceItemId:item.row.evidence_item_id,reopensEpisodeId:reopened?.episode_id??null});
      await linkMembership(context,opened,item,sourceSet,null,calculation);
    ...
    const patch={severity:calculation.severity,current_confidence:calculation.confidence,current_novelty:Number(calculation.novelty),
    ...
    if(calculation.classification==='DIRECTION_REVERSAL') {
      const reversed=await episodes.reverse(context,{prior:{episodeId:active.episode_id,expectedRevision:active.revision,patch,
        ...
        data:episodeData(context,sourceSet,calculation,asOfUtc),evidenceItemId:item.row.evidence_item_id}});
      await linkMembership(context,reversed,item,sourceSet,null,calculation);await semantic(context,reversed,'OPENED',calculation);
    ...
    await linkMembership(context,current,item,sourceSet,null,calculation);
  ...
  async function analyzeMetric(context,request) {
    return core.run(context,async()=>{
      ...
      const identity={...baseIdentity,direction:observationCalculation.direction??'UNKNOWN'},active=observationCalculation.direction?await activeEpisode(context,identity):null;
      ...
      const episode=await episodeForEvidence(context,{identity:baseIdentity,active,sourceSet,calculation,item:evidence.item,asOfUtc:request.asOfUtc});
  ...
  async function associationHypothesis(context,hypothesis,asOfUtc) {
    const fields=['factor','outcomeMetric','lagDays','outcomeSources','journalFactSources','coverageSources'];
    ...
    const refs=[...hypothesis.outcomeSources,...hypothesis.journalFactSources,...hypothesis.coverageSources];
    if(new Set(refs).size!==refs.length)fail('PHASE4_ASSOCIATION_SOURCES_DUPLICATE');
    const resolved=await core.revalidateSources(context,refs),outcomeCount=hypothesis.outcomeSources.length,
      factCount=hypothesis.journalFactSources.length;
    const outcomes=resolved.slice(0,outcomeCount).map(source=>sourceValue(hypothesis.outcomeMetric,source,context,asOfUtc));
    const facts=resolved.slice(outcomeCount,outcomeCount+factCount),coverage=resolved.slice(outcomeCount+factCount);
    ...
    const days=outcomes.map(outcome=>{
      const factorDate=addDays(outcome.healthDate,-hypothesis.lagDays);
      ...
      return {healthDate:outcome.healthDate,factorHealthDate:factorDate,outcome:outcome.value,
        quality:'AVAILABLE',exposureState,outcomeSource:[outcome.sourceType,outcome.sourceId,outcome.sourceVersion],
        exposureSources:[...exposedFacts,...negativeFacts].map(source=>[source.type,source.id]).concat(coverage.filter(source=>parseFactorKeys(source.row).includes(hypothesis.factor)
          &&source.row.health_date_start<=factorDate&&source.row.health_date_end>=factorDate).map(source=>[source.type,source.id]))};
    });
    const first=evaluateJournalAssociation({factor:hypothesis.factor,outcomeMetric:hypothesis.outcomeMetric,days,asOfUtc});
    const classified=days.filter(day=>day.exposureState!=='UNKNOWN'),xs=classified.map(day=>day.exposureState==='EXPOSED'?1:0),
      ys=classified.map(day=>day.outcome),rawSignificance=first.candidate?pValue(pearson(xs,ys),classified.length):null;
    return {hypothesis,refs,outcomes,days,rawSignificance,
  ...
  async function reviseContradiction(context,hypothesis,analysis,item) {
    if(!analysis.repeated)return null;
    ...
    if(existing.row.status==='WEAKENED'||existing.row.status==='HYPOTHESIS')return insights.transition(context,{...base,status:'RETIRED',
      disposition:'REFUTED',supportingEvidenceIds:[],reason:'REFUTED'});
    return insights.transition(context,{...base,status:'WEAKENED',supportingEvidenceIds:[],reason:'CONTRADICTORY_EVIDENCE'});
  }
  async function updateInsight(context,hypothesis,analysis,item) {
    ...
    const contradiction=await reviseContradiction(context,hypothesis,analysis,item);
    ...
    if(!current)current=await insights.create(context,{identity,claim,evidenceContractVersion:INTELLIGENCE_VERSIONS.evidenceContract,
      ...
      expiresAt:new Date(core.now().getTime()+phase4Metric(hypothesis.outcomeMetric).evidenceExpiryMs).toISOString()});
    if(analysis.repeated&&current.row.status==='HYPOTHESIS')current=await insights.transition(context,{insightId:current.row.id,
    ...
    else if(analysis.insightSupporting&&current.row.status==='EMERGING') {
  ...
  async function analyzeAssociationFamily(context,request) {
    return core.run(context,async()=>{
      ...
      const prepared=[];for(const hypothesis of request.hypotheses)prepared.push(await associationHypothesis(context,hypothesis,request.asOfUtc));
      const adjusted=benjaminiHochberg(prepared.map((value,index)=>({key:`${value.hypothesis.factor}:${value.hypothesis.outcomeMetric}:${value.hypothesis.lagDays}:${index}`,
      ...
      const familyManifest={manifest_version:'phase4-association-family-input-v1',multiple_testing_family:request.multipleTestingFamily,
        as_of_utc:request.asOfUtc,hypotheses:finalized.map(value=>({factor:value.hypothesis.factor,outcome_metric:value.hypothesis.outcomeMetric,
          lag_days:value.hypothesis.lagDays,days:value.days,raw_significance:value.rawSignificance,adjusted_significance:value.analysis.adjustedSignificance,
          replication_windows:value.replicationWindows}))};
      const outputs=[],runs=[];
      for(let index=0;index<finalized.length;index+=1) {
        const value=finalized[index],analysis=value.analysis,hypothesis=value.hypothesis,manifest={...familyManifest,focus_index:index},
          manifestJson=canonicalJson(manifest),inputHash=keys.lookup(['phase4-association-input-v1',context.userId,
            ...
            context.executionMode,'JOURNAL_ASSOCIATION',request.multipleTestingFamily,index,request.asOfUtc,context.inputGeneration,inputHash]);
        let runRow=(await client.execute({sql:'SELECT * FROM evidence_runs WHERE user_id=? AND execution_mode=? AND deterministic_run_key=?',
          args:[context.userId,context.executionMode,runKey]})).rows[0],run;
        if(runRow) {
          ...
          const starts=value.outcomes.map(outcome=>outcome.observedAt).sort();
          run=await entities.append(context,'evidence_runs',{deterministic_run_key:runKey,
            ...
            method:'JOURNAL_ASSOCIATION',window_start_utc:starts[0]??request.asOfUtc,window_end_utc:starts.at(-1)??request.asOfUtc,
          ...
          run=await entities.completeEvidence(context,run.row.run_id,{sample_count:analysis.eligibleObservationDays,exclusion_count:0,
            ...
            missingness_json:{factor:hypothesis.factor,reason_codes:analysis.reasonCodes},multiple_testing_family:request.multipleTestingFamily});
        }
        runs.push(run);
        const itemKey=keys.lookup(['phase4-evidence-item-v1',runKey,index,
        ...
        const insight=await updateInsight(context,hypothesis,analysis,item);outputs.push(Object.freeze({analysis,item,insight}));
      }
      return Object.freeze({runs:Object.freeze(runs),items:Object.freeze(outputs)});
    });
  }
  async function expireInsight(context,{insightId}) {
    return core.run(context,async()=>{
      if(context.executionMode!=='SHADOW')fail('PHASE4_INTELLIGENCE_SHADOW_ONLY');
      ...
      if(!current.row.expires_at||current.row.expires_at>core.timestamp())fail('PHASE4_INSIGHT_NOT_EXPIRED');
      ...
      return insights.transition(context,{insightId,expectedRevision:current.row.current_revision,status:'RETIRED',disposition,
        claim:current.row.statement,supportingEvidenceIds:supporting,reason:disposition});
    });
  }
  async function expireEpisode(context,{episodeId}) {
    return core.run(context,async()=>{
      if(context.executionMode!=='SHADOW')fail('PHASE4_INTELLIGENCE_SHADOW_ONLY');
      ...
      if(!row.expires_at||row.expires_at>core.timestamp())fail('PHASE4_EPISODE_NOT_EXPIRED');
      ...
      await episodes.revise(context,{episodeId,expectedRevision:row.revision,toState:'EXPIRED',patch:{latest_evidence_item_id:row.latest_evidence_item_id},
        sourceRefs:[item.ref],reasonCode:'WINDOW_EXPIRED',resolutionHoldMs:phase4Metric(row.subject_key).resolutionHoldMs});
