...
test('probe 結果寫進 Turso，可以再讀回來', async () => {
  ...
  try {
    ...
    const { scopeErrors } = await probeCapabilities({ db, whoop, userId: U, timezone: TZ, days: 14, expectedLifecycleGeneration: 1 });
    ...
    await probeCapabilities({ db, whoop, userId: U, timezone: TZ, days: 14, expectedLifecycleGeneration: 1 });
...
test('probe 不會因為缺 scope 而整個失敗', async () => {
  ...
  try {
    ...
    const { entries } = await probeCapabilities({ db, whoop, userId: U, timezone: TZ, days: 10, expectedLifecycleGeneration: 1 });
