...
test('N: 寫進 DB 並讀回來', async () => {
  ...
  try {
    ...
    const saved = await saveEvent(db, r.event, { now: NOW, timezone: TZ });
    ...
    assert.equal(await db.countJournalEvents(), 1);
...
test('★ P: 完全沒有資料時回完整報告，不是 error', async () => {
  const { db, cleanup } = await freshDb();
  try {
    const r = await buildDataQualityReport({ db, timezone: TZ, now: NOW });
...
test('P: 有資料時報告涵蓋率與缺漏天數', async () => {
  ...
  try {
    ...
    const sync = createSync({ db, whoop, timezone: TZ, now: new Date('2026-09-01T00:00:00Z') });
...
test('P: 某個查詢失敗不會讓整份報告失敗', async () => {
  ...
  const r = await buildDataQualityReport({ db: broken, timezone: TZ, now: NOW });
...
test('★ R: snapshot 只建底座，score 一定是 null（不算生理年齡）', async () => {
  ...
  try {
    ...
    const snaps = await db.getHealthspanSnapshots();
    ...
    const metrics = await db.getLatestHealthspanMetrics();
...
test('★ Y: belief revision —— 舊版本保留、版本鏈可回溯', async () => {
  const { db, cleanup } = await freshDb();
  try {
    const { id, status } = await recordInsight(db, {
    ...
    const rev = await reviseInsight(db, id, {
    ...
    const next = await db.getInsight(rev.id);
    ...
    const chain = await db.getInsightHistory(rev.id);
...
test('Y: 證據變弱 → WEAKENED；狀態沒變則不開新版本', async () => {
  ...
  try {
    ...
    // 同樣強度 → 只是重新確認
    const same = await reviseInsight(db, id, { sampleCount: 46, effectSize: 0.8 }, { now: NOW });
    assert.equal(same.changed, false, '狀態沒變就不要開新版本');
    assert.equal(same.id, id);

    // 效果消失 → WEAKENED
    const weak = await reviseInsight(db, id, { sampleCount: 60, effectSize: 0.05 }, { now: NOW });
...
test('Y: retire 之後不能再修改；HYPOTHESIS 不算「相信」', async () => {
  const { db, cleanup } = await freshDb();
  try {
    const a = await recordInsight(db, {
    ...
    const b = await recordInsight(db, {
      insightType: 'association', subject: 'b', statement: 'strong',
      sampleCount: 50, effectSize: 0.9,
    }, { now: NOW });

    const beliefs = await activeBeliefs(db);
    assert.equal(beliefs.length, 1, '★ 只有證據夠的才算「相信」');
    assert.equal(Number(beliefs[0].id), b.id);

    assert.equal((await retireInsight(db, b.id, { now: NOW })).ok, true);
    assert.equal((await retireInsight(db, b.id, { now: NOW })).ok, false, '不能重複退休');
    assert.equal((await reviseInsight(db, b.id, { sampleCount: 60, effectSize: 0.9 })).ok, false);
  } finally { db.close(); cleanup(); }
});
...
test('Z: 實驗生命週期 DRAFT → RUNNING → COMPLETED', async () => {
  const { db, cleanup } = await freshDb();
  try {
    const c = await createExperiment(db, {
      name: '睡前不喝咖啡', hypothesis: '深睡會變多',
      intervention: '14:00 後不攝取咖啡因', targetMetrics: ['deep_sleep', 'recovery'],
    }, { now: NOW });
    assert.equal(c.ok, true);

    assert.equal((await db.getExperiment(c.id)).status, EXPERIMENT_STATUS.DRAFT);

    const s = await startExperiment(db, c.id, {
      startDate: '2026-08-15', baselineStart: '2026-08-01', baselineEnd: '2026-08-14', now: NOW,
    });
    assert.equal(s.ok, true);
    assert.equal((await db.getExperiment(c.id)).status, EXPERIMENT_STATUS.RUNNING);

    // 不能重複啟動
    assert.equal((await startExperiment(db, c.id, { startDate: '2026-08-20' })).ok, false);

    const done = await completeExperiment(db, c.id, { endDate: '2026-08-28', now: NOW });
    assert.equal(done.ok, true);
    assert.equal((await db.getExperiment(c.id)).status, EXPERIMENT_STATUS.COMPLETED);
  } finally { db.close(); cleanup(); }
});
...
test('Z: 缺必要欄位會被拒絕', async () => {
  const { db, cleanup } = await freshDb();
  try {
    assert.equal((await createExperiment(db, { name: '' })).ok, false);
    assert.equal((await createExperiment(db, { name: 'x' })).ok, false, '沒有 target metrics');
    assert.equal((await startExperiment(db, 999, { startDate: 'x' })).ok, false, '不存在的實驗');
  } finally { db.close(); cleanup(); }
});
...
test('Z: analyzeExperiment 會把結果存回 DB', async () => {
  ...
  try {
    ...
    const res = await analyzeExperiment(db, c.id, rows, { now: NOW });
    assert.equal(res.ok, true);
    const stored = await db.getExperiment(c.id);
